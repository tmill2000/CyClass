...
describe('mediaService', () => {
    ...
    describe('metadataForDownload',  () => {
        it('should return', async () => {
            jest.spyOn(db, 'runQuery').mockResolvedValueOnce([{ recieved: true, file_type: 'png', user_in_course: true }]);
            expect(await mediaService.metadataForDownload(1, 2)).toEqual([{ recieved: true, file_type: 'png', user_in_course: true }])
        })
        it('should throw error', async () => {
            jest.spyOn(db, 'runQuery').mockRejectedValue(new Error());
