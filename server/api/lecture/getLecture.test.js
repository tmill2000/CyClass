...
describe("getLecture", () => {
    ...
    describe("getLecture", () => {
        ...
        it("should return 200 for success", async () => {
            ...
            jest.spyOn(lectureService, "getLecture").mockResolvedValueOnce([
                { lecture_id: 1, course_id: 1, title: "foo" }
            ]);
            await getLecture(req, res);
    ...
    describe("getLecturesByCourseId", () => {
        ...
        it("should return 200 for success", async () => {
            const req = getMockReq({ query: { course_id: 1 } });
            req.session = studentSession;
            jest.spyOn(lectureService, "getLecturesByCourseId").mockResolvedValueOnce([
                { lecture_id: 1, course_id: 1, title: "foo" }
            ]);
            await getLecturesByCourseId(req, res);
            expect(res.status).toBeCalledWith(200);
